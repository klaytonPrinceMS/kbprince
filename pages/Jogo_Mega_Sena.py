...
st.set_page_config(page_title=NOME_SISTEMA, page_icon="🍀", layout="wide", initial_sidebar_state='expanded',
                   menu_items={"About": LINK_PESSOAL})
st.markdown(
    f"""    <style>    .main-title {{text-align: center; color: {COR_QUENTE}; font-weight: bold; margin-bottom: 20px;}}    .stButton>button {{width: 100%; font-weight: bold; border-radius: 10px; height: 45px;}}    .footer-text {{text-align: center; padding: 20px; color: #888; font-size: 14px;}}    .crisp-box {{background-color: #f0f2f6; padding: 20px; border-radius: 10px; border-left: 5px solid {COR_FRIO}; margin-bottom: 10px;}}    </style>    """,
...
def executar_agnes(df_bin_f, n_clusters=4, metric='euclidean', linkage='ward'):
    ...
    model = AgglomerativeClustering(n_clusters=n_clusters, metric=metric,
                                    linkage=linkage)  # Agora o linkage é um parâmetro
...
def realizar_predicao_mlp_custom(df_bin_f, c_ini, c_fim):
    ...
    model = MLPRegressor(hidden_layer_sizes=(120, 80, 40), activation='relu', solver='adam', alpha=0.001,
                         learning_rate='adaptive', max_iter=1000, early_stopping=True, validation_fraction=0.1,
                         random_state=42)
...
def realizar_predicao_rf_custom(df_bin_f, c_ini, c_fim, n_estimators=100, max_depth=10):
    ...
    base_classifier = RandomForestClassifier(n_estimators=n_estimators, max_depth=max_depth, random_state=42,
                                             n_jobs=None)
    ...
    if df_bruto is not None and df_binario is not None:
        ...
        try:
            ...
            with tabs1[2]:
                ...
                fig.add_trace(
                    go.Scatter(x=df_par_soma['Concurso'], y=df_par_soma['Soma'], mode='lines', name='Soma das Dezenas',
                               line=dict(color='#1f77b4')))  # Cor azul padrão
                # Adiciona a linha da Média
                fig.add_hline(y=soma_media, line_dash="dash", line_color="#ff7f0e", annotation_text="Média",
                              annotation_position="bottom right")  # Laranja #ff7f0e
                # Adiciona a linha do Limite Superior
                fig.add_hline(y=limite_superior, line_dash="dot", line_color="#d62728", annotation_text="Média +1σ",
                              annotation_position="bottom right")  # Vermelho #d62728
                # Adiciona a linha do Limite Inferior
                fig.add_hline(y=limite_inferior, line_dash="dot", line_color="#2ca02c", annotation_text="Média -1σ",
                              annotation_position="bottom right")  # Verde #2ca02c
                # Ajustes finais de layout do gráfico
                fig.update_layout(title="Evolução da Soma das Dezenas com Bandas de Média", xaxis_title="Concurso",
                                  yaxis_title="Soma", showlegend=True)
            ...
            with tabs1[3]:
                ...
                fig = px.bar(df_plot, x='Número', y='Score', color='Categoria',
                             title="Extremos de Tendência: Dezenas Quentes vs. Frias",
                             labels={'Número': 'Dezena', 'Score': 'Pontuação de Tendência'}, color_discrete_map={
                        'Quente': COR_QUENTE,
                        'Frio': COR_FRIO
                    })
        ...
        try:
            ...
            with tabs2[2]:
                ...
                try:
                    frequent_itemsets = fpgrowth(df_assoc_input, min_support=min_sup,
                                                 use_colnames=True) if alg_assoc == "FP-Growth" else apriori(
                        df_assoc_input, min_support=min_sup, use_colnames=True)
                        ...
                        if not rules.empty:
                            ...
                            st.dataframe(
                                rules[['antecedents', 'consequents', 'support', 'confidence', 'lift']].sort_values(
                                    'lift', ascending=False), width="stretch")
                        else:
        ...
        try:
            ...
            with tabs3[1]:
                ...
                if tipo_cluster == "Concursos (Padrões de Sorteio)":
                    ...
                    for i in range(n_clusters):
                        st.write(
                            f"**Cluster {i}:** {df_cluster_dez[df_cluster_dez['Cluster'] == i]['Número'].tolist()}")
            with tabs3[2]:
                ...
                if st.button("Executar AGNES",
                             key="agnes_execute_button"):  # Adicionar key ao botão também é uma boa prática
            ...
            with tabs3[3]:
                ...
                if st.button("Executar Mineração Hierárquica"):
                    labels = executar_agnes(df_bin_f, n_h) if metodo_h == "AGNES (Agrupar)" else executar_diana(
                        df_bin_f, n_h)
                    df_bin_f['Cluster_H'] = labels
                    fig_h = px.scatter(df_bin_f, x="Concurso", y="Cluster_H", color=labels.astype(str),
            ...
            with tabs3[4]:
                ...
                c_start = st.number_input("Concurso Inicial:", min_hist, max_hist, max(min_hist, max_hist - 50),
                                          key="dendro_start")
            ...
            with tabs3[5]:
                ...
                m3.metric("Concursos no Ruído", ruidos, delta=f"{(ruidos / total) * 100:.1f}%" if total > 0 else "0.0%",
                          delta_color="inverse")
                fig_db_view = px.scatter(df_bin_f, x="Concurso", y="Cluster_DBSCAN",
                                         color=df_bin_f["Cluster_DBSCAN"].apply(
                                             lambda x: "Ruído" if x == -1 else "Padrão"),
        ...
        try:
            ...
            with tabs4[2]:
                st.subheader("🔍 Analisador de Jogos Personalizados")
                dezenas_user_2 = st.multiselect("Selecione de 4 a 15 DEZENAS:", list(range(1, 61)),
                                                key='multiselect_analisador_2')
                if 4 <= len(dezenas_user_2) <= 15:
                    ...
                    c1, c2, c3 = st.columns(3)
                    score_jogo = sum(
                        df_ranking[df_ranking['Número'] == n]['Score'].values[0] for n in dezenas_user_2) / len(
                        dezenas_user_2)
                    c1.metric("Score Médio SIPP", f"{score_jogo:.2f}")
                    c2.metric("Vezes Premiado (4+)", len(df_ocorr))
                    prob = (len(df_ocorr) / len(df_bruto)) * 100 if len(df_bruto) > 0 else 0
            ...
            with tabs4[3]:  # Esta é agora a sua aba unificada de "Predição com IA"
                ...
                ini_ia = col_ia1.number_input("Treinar a partir do:", c_min_ia, c_max_ia - 1,
                                              max(c_min_ia, c_max_ia - 500),
                ...
                st.info(
                    f"🎯 **Objetivo:** Prever o comportamento do concurso **{proximo_alvo}** usando **{modelo_escolhido}**.")
                    ...
                    with st.spinner(spinner_message):
                        ...
                        if probs is not None and len(probs) == 60:
                            ...
                            st.plotly_chart(fig_ia,
                                            width="stretch")  # use_container_width=True trocado por po width=Stretch
                        else:
        ...
        try:
            ...
            with tabelaBasica[0]:
                ...
                df_view = df_bruto_f[
                    ['Bola1', 'Bola2', 'Bola3', 'Bola4', 'Bola5', 'Bola6', 'Concurso', 'Data do Sorteio']].iloc[
                    ::-1].copy()
